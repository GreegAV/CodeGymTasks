...
It should return a chicken breed corresponding to the continent that the chicken came from.
...
4. The NorthAmericanHen, EuropeanHen, AsianHen, and AfricanHen classes
must inherit the Hen class and be located in separate files.
5. The NorthAmericanHen, EuropeanHen, AsianHen, and AfricanHen classes
must implement the getMonthlyEggCount method, which should return the number
of eggs per month laid by the corresponding type of chicken.
6. The NorthAmericanHen, EuropeanHen, AsianHen, and AfricanHen must override
the parent class's getDescription method so that the returned string looks like this:
<parent class>.getDescription() + "I come from <continent>. I lay <n> eggs a month."
where <continent> is the name of the continent, and <n> is the number of eggs per month.
7. The getHen method must be implemented in the HenFactory class and must return the
chicken type that corresponds to the passed continent.
