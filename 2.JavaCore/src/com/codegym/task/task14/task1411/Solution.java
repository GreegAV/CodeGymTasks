...
2. The main method should stop reading strings from the keyboard as soon as the entered string does not match
one of the allowed strings ("user", "loser", "coder", "programmer").
3. For each correctly entered string ("user", "loser", "coder", "programmer"), call the doWork method with the
appropriate Person object as the argument.
...
public class Solution {
    public static void main(String[] args) throws Exception {
        ...
        correctStrings.add("user");
        correctStrings.add("loser");
        correctStrings.add("coder");
        correctStrings.add("programmer");
        // Here's the loop for reading the keys. Item 1
        while (true) {
            key = reader.readLine();
            if (!correctStrings.contains(key))
                break;
