...
2. In the DrinkMaker class, declare and implement the void makeDrink() method, which prepares a beverage
in the following order: select a cup, add ingredients, pour liquid.
3. In separate files, create LatteMaker and TeaMaker classes that inherit DrinkMaker.
4. Assign the following phrases among all the methods in the LatteMaker and TeaMaker classes,
with different phrases for the different methods.
...
3. The DrinkMaker class must implement the makeDrink method, which calls this class's abstract methods
in the order specified by the task conditions.
